...
var appServer = function(config) {
			...
			try {
				...
				if (false!==config.debug) {
					self.express.get(endpoint,function(req,res) {
						...
						else if (typeof req.param('page')=="help") {
							res.set('Content-Type', 'text/plain').send(app.help());
						}						
						else {
							//res.render('test',{"app":app,"schema":app.schema(),"utterances":app.utterances(),"intents":app.intents});
							//Gnagna changed this line

				res.render('index', {
					title : 'Bar Lola @ Today',
					matches : null,
					loggedIn : true,
					time : 'Today',
					state : 'ended',
					test: req.param("location") + "@" + req.param("time")
				});
	...
	self.start = function() {
		...
		//Gnagna changed this:
		//self.express.use(require('stylus').middleware(__dirname + '/public'));
